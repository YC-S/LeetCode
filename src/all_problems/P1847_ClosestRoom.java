...
public class P1847_ClosestRoom {
  public int[] closestRoom(int[][] rooms, int[][] queries) {
    int n = rooms.length, k = queries.length;
    Integer[] indexes = new Integer[k];
    for (int i = 0; i < k; i++) indexes[i] = i;
    Arrays.sort(
        rooms, (a, b) -> Integer.compare(b[1], a[1])); // Sort by decreasing order of room size
    Arrays.sort(
        indexes,
        (a, b) ->
            Integer.compare(
                queries[b][1], queries[a][1])); // Sort by decreasing order of query minSize
    ...
    for (int index : indexes) {
      while (i < n
          && rooms[i][1]
              >= queries[index][1]) { // Add id of the room which its size >= query minSize
