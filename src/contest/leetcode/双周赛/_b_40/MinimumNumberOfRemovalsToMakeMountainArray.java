...
public class MinimumNumberOfRemovalsToMakeMountainArray {
    ...
    public static int minimumMountainRemovals(int[] nums) {
        int i = 1, maxIndex = 1, max = nums[1], minIndex = 0, min = nums[0];
        ...
        for (int j = minIndex; j <= maxIndex - 1; ++j) {
            if (queue.isEmpty() || nums[j] < nums[j + 1] && queue.peekLast() < nums[j]) {
                queue.offerLast(nums[j]);
            }
        }
        queue.offerLast(max);
        for (int j = maxIndex + 1; j < nums.length - 1; ++j) {
            if (queue.isEmpty() || nums[j] > nums[j + 1] && queue.peekLast() > nums[j]) {
                queue.offerLast(nums[j]);
            }
        }
        queue.offerLast(nums[nums.length - 1]);
